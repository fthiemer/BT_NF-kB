...
def create_QS_dict(floaty = True):
    ...
    for gene in geneDict:
        ...
        for i, BS in enumerate(geneDict[gene]["BS_data"]):
            ...
            RELA_cW_sum += int(BS["RELA_central_BS_W"]) # werden in vorherigen Files zusammengefasst, je nach File das get_gene_BS_sum_dict benutzt
...
for QS in QS_Dict:
    bi_vs_all("Comparison based on " + str(QS), str("Figure_7_" + str(QS) + "_comparison_bi_vs_exp_val"), QS_Dict[QS]["Yes"], QS_Dict[QS]["No"])
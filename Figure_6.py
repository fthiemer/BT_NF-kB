#creating figure 4
#ATTENTION: This script requires the geckodriver binary to be in in a folder in the scripts directory and must be named geckodriver
...
docuFile = openWriteableCSV(os.path.join(".","data", "docuFileFigure4.txt"))
#check whether lille and boston refs are there from another try 
if( os.path.exists(os.path.join(".", "important_source_files", "updatedBostonRefs.txt")) and
	os.path.exists(os.path.join(".", "important_source_files", "updatedLilleRefs.txt"))):
	bostonRefFile = openReadableCSV(os.path.join(".", "important_source_files", "updatedBostonRefs.txt"))
	bostonRefsList = []
	for refseq in bostonRefFile:
		bostonRefsList.append(refseq[0])
	lilleRefFile = openReadableCSV(os.path.join(".", "important_source_files", "updatedLilleRefs.txt"))
	lilleRefsList = []
	...
	bostonRefFile = openWriteableCSV(os.path.join(".", "important_source_files", "updatedBostonRefs.txt"))
	for refseq in bostonRefsList:
		bostonRefFile.writerow([refseq])
	lilleRefFile = openWriteableCSV(os.path.join(".", "important_source_files", "updatedLilleRefs.txt"))
	for refseq in lilleRefsList:
		lilleRefFile.writerow([refseq])
#create refseq to Ensembl dictionary
ref2EnsemblFile = openReadableCSV(os.path.join(".", "important_source_files", "gene2ensembl.txt"))
ref2EnsDict = {}
category = {}
...
def getEnsemblFromGeneSymbol(GeneSymbol):
	...
	if decoded != []:
		...
		restAPIEnsemblSet = set()
	crossRefFile = openReadableCSV(os.path.join(".","important_source_files","20180806_EPD_data_v6",
...
if( os.path.exists(os.path.join(".", "seu_comp_data", "seuEnsembls.txt"))):
	seuEnsemblList = []
	seuEnsemblFile = openReadableCSV(os.path.join(".", "seu_comp_data", "seuEnsembls.txt"))
	for row in seuEnsemblFile:
		seuEnsemblList.append(row[0])
else:
	seu_gNs = returnReadableCSVList(os.path.join(".", "seu_comp_data", "symbol_checker_results_seu.txt"))[1:]
	seuEnsemblFile = openWriteableCSV(os.path.join(".", "seu_comp_data", "seuEnsembls.txt"))
		...
		if i[1] != "Unmatched":
			...
			for j in [currentEnsemblID]:
				if (count+1) == len([currentEnsemblID]) or len([currentEnsemblID]) == 1:
					...
					Ensembls += j + "_%_"
				count += 1
			# seuFile.writerow([i[0], i[2], Ensembls, i[4]])
			if count > 1:
				print("more than one!")
				input()
...
if( os.path.exists(os.path.join(".", "data", "biEnsdembls.txt"))):
	biEnsemblList = []
	...
	def extractEPDGeneName(PromoterID):
		...
		if EPDName==[]:
			...
			return EPDName[0]
	biAllData = openReadableCSV(os.path.join(".", "important_source_files", "20180809_EPD_data_v6_analyzed", 
		"all_data.txt"))
	...
	for geneName in biGeneNameSet:
		...
		#biGeneNameFile gets checked with: https://www.genenames.org/cgi-bin/symbol_checker
